...
<li>Now follow the usual TDD process to update tests and code. The "expected" values for the unit tests are in the fixture (ATSRawDataTransformerTestFixtureBase & child classes for the year) and the "actual" values are in the newly copied and renamed folder. Change unit tests first and only amend the code (add any new fields, update formulae etc) once have failing tests.
