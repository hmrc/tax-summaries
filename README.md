...
<h3>Making the changes locally</h3>
...
<li>Now follow the usual TDD process to update tests and code. <ul><li>The "expected" values for the unit tests are in ATSRawDataTransformerTestFixtureBase & ATSRawDataTranformer*Spec classes.</li><li>The "actual" values are in the ATSCalculations* classes.</li><li>It's best to use TDD, only amending the code (add any new fields, update formulae etc) once we have a failing test.</li></ul>
...
<li>Test service locally in browser. Make sure ATS FE/ BE are started with "-Dapplication.router=testOnlyDoNotUseInAppConf.Routes" (will be needed when testing SME test tool below)
<li>Test that the calculations appear using the SME test tool (/annual-tax-summary/test-only/enterSearch).</li>
</ol>

<h3>After successfully testing the changes locally</h3>
