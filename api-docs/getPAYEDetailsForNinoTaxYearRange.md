# Get PAYE details for nino and tax year range

Returns the paye tax and NI details for a nino and range of tax years as well as the government spend figures.

Calls to this API must be made by an authenticated and authorised user (at least confidence level 50) with the matching id.

**URL**: `/:NINO/:YEAR_FROM/:YEAR_TO/paye-ats-data`
...
| YEAR_FROM      | String | The start tax year |                             |
| YEAR_TO        | String | The end tax year   |                             |
...
/OO123456B/2023/2025/paye-ats-data
...
The response body returns an array of the paye tax and NI details and the government spend figures by tax year.

**Response Body**

An array of JSON objects, one per tax year in the range. The fields included will depend on the tax year and country where the tax has been paid (England, Scotland or Wales). All the sections below though should be present.
...
***The response will be an array of JSON objects. The structure of the JSON objects are the same as the endpoint for one tax year. ***
