...
trait ATSCalculations2026 extends ATSCalculations {
  ...
  override def otherAllowances: Amount =
    (
      get(EmploymentExpenses) +
        ...
        get(BpaAllowance) +
        get(BPA) +
        get(ForeignIncomeRelief) +
        get(ForeignEarningReliefRestricted)
    ).roundAmountUp()

  override def otherAdjustmentsIncreasing: Amount =
    get(TemporaryRepatriationFacility) +
