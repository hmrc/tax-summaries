...
class ATSCalculationsScottish2026(
  val selfAssessmentAPIResponse: SelfAssessmentAPIResponse,
  val taxRates: Map[String, Rate]
) extends ATSCalculations2026 {

  override def scottishIncomeTax: Amount = Amount.empty("scottishIncomeTaxScottish2026")

  override def savingsRate: Amount = Amount.empty("savingsRateScottish2026")

  override def savingsRateAmount: Amount = Amount.empty("savingsRateAmountScottish2026")

  override def basicRateIncomeTaxAmount: Amount = Amount.empty("basicRateIncomeTaxAmountScottish2026")

  override def higherRateIncomeTaxAmount: Amount = Amount.empty("higherRateIncomeTaxAmountScottish2026")

  override def additionalRateIncomeTaxAmount: Amount = Amount.empty("additionalRateIncomeTaxAmountScottish2026")

  override def basicRateIncomeTax: Amount = Amount.empty("basicRateIncomeTaxScottish2026")

  override def higherRateIncomeTax: Amount = Amount.empty("higherRateIncomeTaxScottish2026")

  override def additionalRateIncomeTax: Amount = Amount.empty("additionalRateIncomeTaxScottish2026")
  ...
  override def scottishAdvancedRateTax: Amount =
    getWithDefaultAmount(TaxOnPayScottishAdvancedRate) + get(TaxOnRedundancySar) + includePensionTaxForRate(
      taxRates.getOrElse(ScottishAdvancedRate, Rate.empty)
    )
