...
case class ATSRawDataTransformer(rawJsonFromStub: JsValue, rawTaxPayerJson: JsValue, UTR: String, taxYear: Int) {
  ...
  private def noAtsResult: AtsMiddleTierData =
    AtsMiddleTierData(taxYear, None, None, None, None, None, None, None, None, Option(AtsError("NoAtsError")))
  ...
  private def hasIncomeAndCapitalGainsLiability =
    !(createTotalCapitalGainsTax + createTotalIncomeTaxAmount).isZeroOrLess
  ...
  private def createIncomeTaxData =
    Option(DataHolder(createTotalIncomeTaxPageBreakdown, createTotalIncomeTaxPageRates, createIncomeTaxStatus))
  ...
  private def createCapitalGainsData =
    Option(DataHolder(createCapitalGainsTaxBreakdown, createCapitalGainsTaxRates, None))
  ...
  private def createYourIncomeBeforeTaxBreakdown =
    Option(
      Map(
        "self_employment_income"   -> createSelfEmployment,
  ...
  private def createSummaryPageBreakdown =
    Option(
      Map(
        "employee_nic_amount"            -> createTotalAmountEmployeeNic,
  ...
  private def createSummaryPageRates =
    Option(Map("total_cg_tax_rate" -> createTotalCgTaxRate, "nics_and_tax_rate" -> createNicsAndTaxTaxRate))

  private def createTotalIncomeTaxPageBreakdown =
    Option(
      Map(
        "starting_rate_for_savings"          -> createStartingRateForSavings,
  ...
  private def createOtherAdjustmentsIncreasing =
    getAmountSum(
      "nonDomChargeAmount",
      "taxExcluded",
      "incomeTaxDue",
  ...
  private def createOtherAdjustmentsReducing =
    (getAmountSum(
      "ctnDeficiencyRelief",
      "topSlicingRelief",
    ...
    ) + createReliefForFinanceCosts).roundAmountUp

  private def createReliefForFinanceCosts = getTliSlpAmountOptVal("reliefForFinanceCosts")

  private def createTotalIncomeTaxAmount =
    createStartingRateForSavingsAmount +
  ...
  private def createSelfEmployment = getAmountSum("ctnSummaryTotalScheduleD", "ctnSummaryTotalPartnership")
  ...
  private def createOtherPension = getAmountSum("atsOtherPensionAmt", "itfStatePensionLsGrossAmt")

  private def createTaxableStateBenefits =
    getAmountSum("atsIncBenefitSuppAllowAmt", "atsJobSeekersAllowanceAmt", "atsOthStatePenBenefitsAmt")

  private def createOtherIncome =
    getAmountSum(
  ...
  private def createTaxableGains = getAmountSum("atsCgTotGainsAfterLosses", "atsCgGainsAfterLossesAmt")
  ...
  private def createTotalCapitalGainsTax =
    createCtnCgDueEntrepreneursRate + createCtnCgDueLowerRate + createCtnCgDueHigherRate - createCapAdjustmentAmt

  private def createPayCapitalGainsTaxOn =
    if (createTaxableGains < createLessTaxFreeAmount) Amount(0.00, "GBP")
    else createTaxableGains - createLessTaxFreeAmount
  ...
  private def createScottishIncomeTax =
    Amount(
      (createCtnIncomeChgbleBasicRate + createCtnIncomeChgbleHigherRate + createCtnIncomeChgbleAddHRate).amount * 0.1,
      "GBP")

  private def createOtherAllowancesAmount =
    getAmountSum(
  ...
  private def createNicsAndTaxPerCurrencyUnit =
    taxPerTaxableCurrencyUnit(createTotalAmountTaxAndNics, createTotalIncomeBeforeTax)
