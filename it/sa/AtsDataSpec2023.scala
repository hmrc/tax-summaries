...
class AtsDataSpec2023 extends SaTestHelper {
    ...
    expected foreach { case (key, expectedValue) =>
      s"return the correct key $key" in new Test {
        server.stubFor(
          WireMock
            .get(urlEqualTo(odsUrl(taxYear)))
            .willReturn(ok(FileHelper.loadFile("2022-23/TC11.json")))
        )
  ...
  "return correct government spending" in new Test {
    ...
    server.stubFor(
      WireMock
        .get(urlEqualTo(odsUrl(taxYear)))
        .willReturn(ok(FileHelper.loadFile("2022-23/TCGovernmentSpend.json"))) // <<-- TODO: Only works when year in this json file is set to 2020 for some reason
    )
