...
class AtsDataSpec2022 extends SaTestHelper {
  ...
  "return correct government spending" in new Test {
    ...
    val expectedValue: GovernmentSpendingOutputWrapper = GovernmentSpendingOutputWrapper(
      2022,
      Map(
        PublicOrderAndSafety       -> SpendData(Amount(2458.25, "GBP"), 4.4),
        BusinessAndIndustry        -> SpendData(Amount(3016.94, "GBP"), 5.4),
        NationalDebtInterest       -> SpendData(Amount(4246.07, "GBP"), 7.6),
        Defence                    -> SpendData(Amount(2849.34, "GBP"), 5.1),
        Health                     -> SpendData(Amount(12738.21, "GBP"), 22.80),
        HousingAndUtilities        -> SpendData(Amount(893.91, "GBP"), 1.60),
        GovernmentAdministration   -> SpendData(Amount(1285, "GBP"), 2.3),
        Environment                -> SpendData(Amount(838.04, "GBP"), 1.5),
        OverseasAid                -> SpendData(Amount(335.22, "GBP"), 0.60),
        Culture                    -> SpendData(Amount(726.3, "GBP"), 1.3),
        OutstandingPaymentsToTheEU -> SpendData(Amount(391.09, "GBP"), 0.7),
        Transport                  -> SpendData(Amount(2625.86, "GBP"), 4.7),
        Welfare                    -> SpendData(Amount(11397.35, "GBP"), 20.40),
        Education                  -> SpendData(Amount(5866.28, "GBP"), 10.50),
        StatePensions              -> SpendData(Amount(6145.63, "GBP"), 11)
      ),
      Amount(6377, "GBP"),
      None
    )
