...
class NpsServiceTest extends BaseSpec with JsonUtil {
  ...
  lazy val transformedDataForLastYear: PayeAtsMiddleTier =
    atsData.transformToPayeMiddleTier(applicationConfig, testNino, currentYear - 1)

  val npsConnector: NpsConnector = mock[NpsConnector]

  class TestService extends NpsService(npsConnector, applicationConfig)

  private val currentYear = 2024

  "getAtsPayeDataMultipleYears" must {
    "return a successful response" in new TestService {
      when(npsConnector.connectToPayeTaxSummary(eqTo(testNino), any())(any()))
        .thenReturn(EitherT.rightT(HttpResponse(200, expectedNpsResponse, Map.empty)))

      val result: Either[UpstreamErrorResponse, List[PayeAtsMiddleTier]] =
        getAtsPayeDataMultipleYears(testNino, List(currentYear, currentYear - 1)).value.futureValue

      result mustBe Right(List(transformedData, transformedDataForLastYear))
    }
    "return an empty list when upstream returns Not Found" in new TestService {
      val response: UpstreamErrorResponse = UpstreamErrorResponse("Not Found", NOT_FOUND, NOT_FOUND)
    ...
    "return a UpstreamErrorResponse in case of UpstreamErrorResponse from Connector" in new TestService {
      ...
      when(npsConnector.connectToPayeTaxSummary(eqTo(testNino), eqTo(currentYear - 2))(any()))
        .thenReturn(EitherT.leftT(response))
