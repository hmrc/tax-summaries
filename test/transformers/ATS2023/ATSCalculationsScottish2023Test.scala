...
import transformers.Scottish.ATSCalculationsScottish2023
import utils.{BaseSpec, JsonUtil}

class ATSCalculationsScottish2023Test extends BaseSpec {
  ...
  class FakeATSCalculationScottish2023(taxSummaryLiability: TaxSummaryLiability)
      extends ATSCalculationsScottish2023(taxSummaryLiability, taxRate)

  def sut(taxSummaryLiability: TaxSummaryLiability = taxSummaryLiability): ATSCalculationsScottish2023 =
    new FakeATSCalculationScottish2023(taxSummaryLiability)

  "Scottish 2023" must {
    "return empty" when {
      "scottishIncomeTax is called" in {
        sut().scottishIncomeTax mustBe Amount.empty("scottishIncomeTaxScottish2023")
      }

      "savingsRate is called" in {
        sut().savingsRate mustBe Amount.empty("savingsRateScottish2023")
      }

      "savingsRateAmount is called" in {
        sut().savingsRateAmount mustBe Amount.empty("savingsRateAmountScottish2023")
      }

      "basicRateIncomeTaxAmount is called" in {
        sut().basicRateIncomeTaxAmount mustBe Amount.empty("basicRateIncomeTaxAmountScottish2023")
      }

      "higherRateIncomeTaxAmount is called" in {
        sut().higherRateIncomeTaxAmount mustBe Amount.empty("higherRateIncomeTaxAmountScottish2023")
      }

      "additionalRateIncomeTaxAmount is called" in {
        sut().additionalRateIncomeTaxAmount mustBe Amount.empty("additionalRateIncomeTaxAmountScottish2023")
      }

      "basicRateIncomeTax is called" in {
        sut().basicRateIncomeTax mustBe Amount.empty("basicRateIncomeTaxScottish2023")
      }

      "higherRateIncomeTax is called" in {
        sut().higherRateIncomeTax mustBe Amount.empty("higherRateIncomeTaxScottish2023")
      }

      "additionalRateIncomeTax is called" in {
        sut().additionalRateIncomeTax mustBe Amount.empty("additionalRateIncomeTaxScottish2023")
      }
    }
