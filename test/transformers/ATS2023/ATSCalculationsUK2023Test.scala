...
import transformers.UK.ATSCalculationsUK2023
import utils.{BaseSpec, JsonUtil}

class ATSCalculationsUK2023Test extends BaseSpec {
  ...
  class FakeATSCalculationUK2023(taxSummaryLiability: TaxSummaryLiability)
      extends ATSCalculationsUK2023(taxSummaryLiability, taxRate)

  def sut(taxSummaryLiability: TaxSummaryLiability = taxSummaryLiability): ATSCalculationsUK2023 =
    new FakeATSCalculationUK2023(taxSummaryLiability)

  "UK 2023" must {
    "return empty" when {
      "scottishIncomeTax is called" in {
        sut().scottishIncomeTax mustBe Amount.empty("scottishIncomeTaxUK2023")
      }
